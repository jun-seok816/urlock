...
function AccessBadge({ level }: { level: AccessLevel }) {
  const label =
    level === "owner" ? "소유자 전용" : level === "team" ? "팀 공유" : "서명 링크";
...
function VaultApp() {
  ...
  const [note, setNote] = useState("클라이언트는 경로를 알 수 없고, 서버가 읽어 스트리밍");
  ...
  const handleUpload = () => {
    ...
    toast.success("메타데이터와 함께 저장했습니다. 이제 서버 엔드포인트와 연결해 보세요.");
  };
  ...
  return (
    <main className="page-shell">
      <header className="hero">
        ...
        <p className="lede">
          업로드 시점에 메타데이터를 기록하고, 파일 경로를 직접 노출하지 않는 흐름을
          포트폴리오로 보여줄 수 있는 샘플입니다.
        </p>
        ...
        <section className="panel upload-panel">
          <div className="panel-head">
            <div>
              ...
              <p className="muted">
                실제 API를 연결할 때는 DB에 메타데이터를 먼저 저장하고, 저장소에는 UUID
                이름으로 보관하세요.
              </p>
            </div>
          ...
          <div className="dropzone">
            <label htmlFor="file-input" className="drop-content">
              ...
              <div>
                ...
                <div className="muted">
                  최대 50MB. 업로드 시 서버에서 바이러스 검사, 확장자 화이트리스트
                  검증을 권장합니다.
                </div>
              </div>
          ...
          <div className="actions">
            ...
            <p className="muted">
              실제 구현: 서버는 파일을 임시 저장 → 검증 → UUID로 이동 → DB에 메타데이터
              기록 → fileId를 클라이언트에 응답합니다.
            </p>
          </div>
        </section>
        ...
        <section className="panel list-panel">
          <div className="panel-head">
            <div>
              ...
              <h2>서버가 인증 후 스트리밍</h2>
              <p className="muted">
                클라이언트는 파일 경로를 알 수 없습니다. 요청 시 fileId로 조회 → 권한
                체크 → 서버가 직접 파일을 읽어 응답합니다.
              </p>
            </div>
          </div>
